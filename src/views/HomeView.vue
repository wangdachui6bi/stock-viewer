  ...
  <div class="app" :class="{ h5: isH5 }">
      ...
      <div class="header-inner">
        ...
        >
          <div class="user-avatar-btn">
            ...
            <span v-if="!isH5" class="user-name">{{
          ...
          <template #dropdown>
            <el-dropdown-menu>
              ...
              <el-dropdown-item :divided="!isAdmin" command="changePwd"
      ...
      <section class="watchlist-section">
        <div class="section-head">
          ...
          <div class="actions">
            ...
            <el-dropdown trigger="click" @command="handleToolsCommand">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="simulation"
                    >模拟盘策略</el-dropdown-item
                  >
                  <el-dropdown-item divided command="trades"
                    >交易记录</el-dropdown-item
                  >
                  <el-dropdown-item command="savedJournals"
    ...
    >
      <div class="trades-toolbar">
        <el-button type="primary" size="small" @click="openAddTrade"
          >新增交易</el-button
        >
        <el-button size="small" :loading="tradesLoading" @click="loadTrades"
          >刷新</el-button
        >
        <span class="muted" style="margin-left: 8px"
          >共 {{ tradesList.length }} 条</span
        >
      </div>
      <el-skeleton
        v-if="tradesLoading && !tradesList.length"
        :rows="6"
        animated
      />
        ...
        <el-table-column label="操作" width="70" align="center">
          <template #default="{ row }">
            <el-button
              ...
              type="danger"
              size="small"
              @click="handleDeleteTrade(row.id)"
              >删除</el-button
            >
          </template>
        </el-table-column>
    ...
    >
      <div class="trades-toolbar">
        <el-button type="primary" size="small" @click="openAddJournal"
          >新建笔记</el-button
        >
        <el-button
          size="small"
          :loading="savedJournalsLoading"
          @click="loadSavedJournals"
          >刷新</el-button
        >
      </div>
      <el-skeleton
        v-if="savedJournalsLoading && !savedJournalsList.length"
        :rows="6"
        animated
      />
      ...
      <div v-else class="journals-list">
        <div v-for="j in savedJournalsList" :key="j.id" class="journal-card">
          <div class="journal-card-head">
            ...
            <div>
              <el-button link size="small" @click="openEditJournal(j)"
                >编辑</el-button
              >
              <el-button
                link
                type="danger"
                size="small"
                @click="handleDeleteJournal(j.id)"
                >删除</el-button
              >
            </div>
          </div>
      ...
      <!-- 新建/编辑笔记 -->
      <el-dialog
        v-model="journalFormModal"
        :title="journalFormEditId ? '编辑笔记' : '新建笔记'"
        width="600px"
        append-to-body
        destroy-on-close
      >
...
import { currentUser, isAdmin, doLogout } from "@/stores/auth";
...
function saveStockPrice() {
  localStorage.setItem(
    uKey(STORAGE_PRICE_BASE),
    JSON.stringify(stockPrice.value),
  );
}
...
function handleToolsCommand(command: string) {
  if (command === "simulation") simulationModal.value = true;
...
async function addCode(code: string) {
  ...
  try {
    const name = stockList.value.find(
...
async function handleDeleteTrade(id: number) {
  try {
    await ElMessageBox.confirm("确认删除此交易记录？", "提示", {
      type: "warning",
    });
  } catch {
    return;
...
async function handleSaveJournal() {
  ...
  const payload = {
    ...
    tags: journalForm.value.tags
      ? journalForm.value.tags.split(",").map((t) => t.trim())
      : undefined,
  };
  try {
...
async function syncWatchlistFromApi() {
  try {
    ...
    groups.value.forEach((g) =>
      g.codes.forEach((c) => groupCodes.add(c.toLowerCase())),
    );
