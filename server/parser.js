...
function calcFixedPriceNumber(open = '0', yestclose = '0', price = '0', high = '0', low = '0') {
  const reg = /0+$/g;
 ...
 */
function parseSinaStockResponse(body, codes) {
  const list = [];
  ...
  for (let i = 0; i < splitData.length - 1; i++) {
    ...
    if (/^(sh|sz|bj)/.test(code)) {
      // A股
      let open = params[1], yestclose = params[2], price = params[3];
      if (Number(price) === 0) price = Number(params[6]) !== 0 ? params[6] : yestclose;
      const high = params[4], low = params[5];
      ...
      let open = params[5], yestclose = params[26], price = params[1];
      const high = params[6], low = params[7];
      fixedNumber = calcFixedPriceNumber(open, yestclose, price, high, low);
      const updown = +price - +yestclose;
      const percent = ((Math.abs(updown) / +yestclose) * 100).toFixed(2);
      item = {
      ...
      // 国内期货
      let name = params[0], open = params[2], high = params[3], low = params[4];
      let price = params[8], yestclose = params[8 + 2], volume = params[8 + 6];
      const stockIndexFuture = /nf_IC|nf_IF|nf_IH|nf_IM|nf_TF|nf_TS|nf_T\d+|nf_TL/.test(code);
      if (stockIndexFuture) {
        name = (params[49] || '').slice(0, -1).replace(/"/g, '');
        open = params[0]; high = params[1]; low = params[2];
        price = params[3]; volume = params[4]; yestclose = params[13];
      }
      fixedNumber = calcFixedPriceNumber(open, yestclose, price, high, low);
      const updown = +price - +yestclose;
      ...
      // 海外期货
      let price = params[0];
      if (Number(price) > Number(params[3]) || Number(price) < Number(params[2])) price = params[2];
      let name = (params[13] || '').endsWith('"') ? params[13].slice(0, -1) : (params[13] || '');
      const open = params[8], high = params[4], low = params[5], yestclose = params[7];
 ...
 * 解析腾讯港股接口返回的 JSON
 */
function parseTencentHKResponse(data, codes) {
  return codes.map((code) => {
    const codeKey = `r_${code}`;
    const arr = data[codeKey];
    const codeNorm = code.startsWith('hk') ? 'hk' + code.substring(2).toLowerCase() : code.toLowerCase();
    if (!arr) return { code: codeNorm, name: 'NODATA', type: 'nodata', percent: '' };
    const price = arr[3], yestclose = arr[4], open = arr[5];
    const high = arr[33], low = arr[34];
