...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const mockFlag =
      String(q.get('mock') || '').toLowerCase() === '1' ||
      String(q.get('mock') || '').toLowerCase() === 'true' ||
      Boolean(body?.mock)
    if (mockFlag) return sendJson(res, 200, AI_SECTOR_MOCK)

    const { mode, topSectorN, topStockN, horizon, riskProfile } = body || {}
