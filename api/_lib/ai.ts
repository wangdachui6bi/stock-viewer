...
export async function callLLM(messages: ChatMessage[]) {
  ...
  const volcBaseUrl = process.env.VOLCENGINE_BASE_URL || 'https://ark.cn-beijing.volces.com/api/v3'
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      throw new Error(`LLM API error${status ? ` (${status})` : ''}: ${e?.message || String(e)}`)
    }
  }
